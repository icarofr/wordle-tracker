...
| Backend  | Go 1.26, `net/http.ServeMux`, oapi-codegen (strict server), sqlx, bcrypt |
| Database | PostgreSQL 18                                                            |
| Testing  | Vitest + MSW (client), moq (server), Venom (integration)                 |
| Build    | Vite + pnpm (client), Go modules (server)                                |
| CI/CD    | GitHub Actions, Docker Compose, Caddy                                    |
...
| `make generate`         | Run all code generators (oapi-codegen + moq + openapi-typescript) |
...
| `make test/integration` | Run Venom API tests via Docker Compose                            |
| `make build`            | Build server binary + client static files                         |
| `make deploy`           | Build client + restart production stack                           |
| `make up`               | Start production stack via Docker Compose                         |
...
```bash
make test/integration   # Docker Compose: Postgres + server + Venom
```
...
17 endpoints defined in `server/api/openapi.yaml`. All under `/api`.
...
server/
  ...
  internal/
    ...
    postgres/           # Database layer (all Store interfaces)
...
docker-compose.yml      # Production (API + Postgres)
docker-compose.test.yml # Test (Postgres tmpfs + API + Venom)
```
