...
func (s *Service) Login(ctx context.Context, input LoginInput) (*SessionUser, string, error) {
	creds, err := s.store.GetCredentialsByEmail(ctx, input.Email)
	if err != nil {
	...
	token, err := s.store.CreateToken(ctx, creds.ID, creds.Email, tokenContext)
	if err != nil {
...
func (s *Service) Register(ctx context.Context, input RegisterInput) (*SessionUser, string, error) {
	...
	user, err := s.store.CreateUser(ctx, input.Name, input.Email, string(hash))
	if err != nil {
		return nil, "", err
	}

	token, err := s.store.CreateToken(ctx, user.ID, user.Email, TokenContextShort)
	if err != nil {
		return nil, "", fmt.Errorf("create token: %w", err)
	}
...
func (s *Service) UpdateAvatar(ctx context.Context, userID int64, avatar string) (*SessionUser, error) {
	user, err := s.store.UpdateAvatar(ctx, userID, avatar)
	if err != nil {
...
func (s *Service) UserByAuthorization(ctx context.Context, authorization string) (*SessionUser, error) {
	...
	user, err := s.store.GetSessionByToken(ctx, token)
	if err != nil {
		if errors.Is(err, ErrSessionNotFound) {
			return nil, nil
		}
