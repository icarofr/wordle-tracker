...
func TestStatsRoundingUsesExactDisplayPrecision(t *testing.T) {
	stats := statsFromScores([]string{"3", "4", "X"})

	if stats.AverageScore != 4.67 {
		t.Errorf("AverageScore = %v, want 4.67", stats.AverageScore)
	}
	if stats.WinPercentage != 66.7 {
		t.Errorf("WinPercentage = %v, want 66.7", stats.WinPercentage)
	}
}
...
func TestCalculateStreak(t *testing.T) {
	tests := []struct {
		...
		{"empty", nil, 0, 0},
		{"single", []int32{100}, 1, 1},
		{"consecutive desc", []int32{105, 104, 103, 102, 101}, 5, 5},
		{"broken streak", []int32{105, 104, 103, 100, 99}, 3, 3},
		{"current broken max longer", []int32{110, 105, 104, 103, 102, 101}, 1, 5},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
