...
func parseWordleInput(input string) (wordleID int32, score string, err error) {
	input = strings.TrimSpace(input)

	if m := _wordleRegex.FindStringSubmatch(input); m != nil {
		numStr := strings.ReplaceAll(m[1], ",", "")
		parsed, parseErr := strconv.ParseInt(numStr, 10, 32)
		wordleID, err = int32(parsed), parseErr
		if err != nil {
	...
	if m := _wordleReplayRegex.FindStringSubmatch(input); m != nil {
		parsed, parseErr := strconv.ParseInt(m[1], 10, 32)
		wordleID, err = int32(parsed), parseErr
		if err != nil {
			return 0, "", ErrInvalidFormat
		}
