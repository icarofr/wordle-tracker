...
const defaultArchiveLimit int32 = 50
...
func (app *application) SubmitWordle(ctx context.Context, request api.SubmitWordleRequestObject) (api.SubmitWordleResponseObject, error) {
	...
	if err != nil {
		...
		case errors.Is(err, wordles.ErrInvalidFormat):
			return nil, badRequest(problemInvalidFormat, "Unrecognizable wordle share text")
		case errors.Is(err, wordles.ErrDuplicateEntry):
			return nil, conflict(problemDuplicateEntry, "You have already submitted this wordle")
		default:
			return nil, err
		}
	}
...
func (app *application) GetArchive(ctx context.Context, request api.GetArchiveRequestObject) (api.GetArchiveResponseObject, error) {
	...
	if request.Params.Limit != nil {
		if *request.Params.Limit < 1 {
			return nil, badRequest(problemInvalidLimit, "invalid limit")
		}
		limit = int32(min(*request.Params.Limit, 100))
	}
	...
	if request.Params.Cursor != nil {
		...
		if err != nil {
			return nil, badRequest(problemInvalidCursor, "invalid cursor")
		}
		beforeID = decoded
	}
...
func (app *application) GetArchiveWordle(ctx context.Context, request api.GetArchiveWordleRequestObject) (api.GetArchiveWordleResponseObject, error) {
	...
	detail, err := app.wordles.ArchiveDetail(ctx, user.ID, int32(request.WordleID), wordles.PlayersFromUsers(allUsers))
	if err != nil {
		return nil, err
	}
...
func (app *application) GetHeadToHead(ctx context.Context, request api.GetHeadToHeadRequestObject) (api.GetHeadToHeadResponseObject, error) {
	...
	if err != nil {
		...
		case errors.Is(err, users.ErrInvalidOpponent):
			return nil, badRequest(problemInvalidOpponent, "Cannot compare with yourself")
...
func (app *application) GetLeaderboard(ctx context.Context, request api.GetLeaderboardRequestObject) (api.GetLeaderboardResponseObject, error) {
	...
	data, err := app.wordles.Leaderboard(ctx, wordles.PlayersFromUsers(allUsers))
	if err != nil {
		return nil, err
	}
...
func encodeCursor(wordleID int32) string {
	return base64.RawURLEncoding.EncodeToString([]byte(strconv.FormatInt(int64(wordleID), 10)))
}
...
func decodeCursor(cursor string) (int32, error) {
	...
	parsed, err := strconv.ParseInt(string(b), 10, 32)
